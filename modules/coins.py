...
async def get_coins_helper(user_id: int, guild_id: int) -> int:
    query = "SELECT COINS FROM MEMBERS WHERE USER_ID=? AND GUILD_ID=?"

    async with DatabaseHandler.get_connection().execute(query, (user_id, guild_id)) as cursor:
...
async def change_coins_helper(user_id: int, guild_id: int, *, num_coins: int) -> None:
    ...
    """
    await DatabaseHandler.get_connection().execute(query, (user_id, guild_id, num_coins))
...
async def message_create(client, message: Message):
    ...
    await change_coins_helper(message.author.id, message.guild.id, num_coins=1)
...
async def coins(client, event, user: ("user", "Specific user") = None):
    ...
    neko_coins = await get_coins_helper(user.id, event.guild.id)
...
async def level(client, event):
    ...
    """
    neko_coins = await get_coins_helper(event.user.id, event.guild.id)
...
async def buy(client, event, item: (ShopItem.item_choices(), "Buy cat items with your Neko coins (NC).")):
    ...
    await change_coins_helper(event.user.id, event.guild.id, num_coins=-selected_item.price)
    return (
